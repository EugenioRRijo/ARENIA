"""Evaluador seguro de expresiones aritméticas trazables.

Las reglas paramétricas del dominio civil (`adapters/civil/reglas.py`) se declaran como texto para
que queden trazables: el informe de auditoría puede mostrar la expresión exacta que produjo cada
cantidad. Este módulo evalúa ese texto sin usar `eval()`: solo permite el subconjunto aritmético
necesario (suma, resta, multiplicación, división, potencia, negación unaria, nombres de parámetro,
constantes numéricas y paréntesis, que el AST ya resuelve como precedencia). Cualquier otro nodo del
árbol sintáctico (llamadas, atributos, índices, comparaciones, comprensiones, etc.) se rechaza.
"""
